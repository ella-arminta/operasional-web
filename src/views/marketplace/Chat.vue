...
const loadConversations = async () => {
	...
	try {
		...
		const response = await axios.get(
			`${baseURL}/chat/conversations/store/${storeId}`
		)
		...
		if (response.data.success) {
			...
			console.error(
				'❌ Error fetching store conversations:',
...
const initializeSocketWithStatus = () => {
	...
	socket.on('new_message', (message: Message) => {
		...
		showNotification(
			'New Message',
			`New message from customer: ${message.content.substring(0, 50)}...`
		)
		// }
	})
...
The app will still work for viewing conversations, but real-time features will be disabled.
	...
	if (
		confirm(
			'Chat service is not available. Real-time features will be disabled. Do you want to continue?'
		)
	) {
	...
	<div class="content min-h-screen" :class="{ 'full-width': smallMenu }">
		...
		<div v-if="currentView === 'list'">
			...
			>
				...
				>
					<svg
						class="animate-spin -ml-1 mr-3 h-5 w-5 text-pinkDark"
			...
			<div v-else class="bg-white rounded-lg shadow-md">
				<div class="divide-y divide-gray-200">
					<div
						...
						class="p-6 hover:bg-gray-50 cursor-pointer tr ansition-colors group"
					>
						<div class="flex items-start space-x-4">
							...
							<div class="flex-shrink-0">
								<svg
									class="w-5 h-5 text-gray-400 group-hover:text-pinkDark transition-colors"
									fill="none"
									stroke="currentColor"
									viewBox="0 0 24 24"
								>
		...
		<div v-else-if="currentView === 'chat'" class="flex flex-col h-screen">
			<!-- Chat Header -->
			<div
				class="bg-white rounded-lg shadow-md p-4 mb-4 flex items-center justify-between"
			>
			...
			<!-- Messages Container -->
			<div
				class="flex-1 bg-white rounded-lg shadow-md overflow-hidden flex flex-col"
			>
				<!-- Loading State -->
				<div
				...
				>
					<div class="text-center">
						<svg
							class="animate-spin h-8 w-8 text-pinkDark mx-auto mb-4"
							xmlns="http://www.w3.org/2000/svg"
							fill="none"
							viewBox="0 0 24 24"
						>
				...
				>
					<div
						v-if="Object.keys(groupedMessages).length === 0"
						class="text-center py-12"
					>
						<div
							class="w-20 h-20 mx-auto mb-6 bg-gray-200 rounded-full flex items-center justify-center"
						>
							<svg
						...
						<p class="text-gray-600 max-w-sm mx-auto">
							...
							They'll receive a notification when you reply.
						</p>
					</div>
					...
					<div
						v-for="(dayMessages, date) in groupedMessages"
						:key="date"
						class="space-y-6"
					>
						<!-- Date Separator -->
						...
						>
							...
							<div class="flex flex-col max-w-xs lg:max-w-md">
								...
								>
									...
									<span
										v-if="message.senderType === 'store'"
										class="ml-1 inline-block"
									>
										<svg
											class="w-3 h-3 inline text-gray-300"
											fill="currentColor"
											viewBox="0 0 20 20"
										>
											<path
				...
				<div class="border-t border-gray-200 bg-white p-4">
					<div class="flex items-end space-x-3">
						...
						<button
							...
							:class="
								!newMessage.trim() ? 'scale-90' : 'scale-100'
							"
						>
							<svg
								v-if="isSending"
