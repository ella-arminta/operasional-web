    ...
    <div class="content min-h-screen" :class="{ 'full-width': smallMenu }">
        ...
        <div class="w-full bg-white h-auto rounded-lg shadow-sm py-3 px-4">
            <form @submit.prevent="submit">
                <FormHeader
                    :title="mode === 'edit' ? 'Edit Voucher' : mode === 'view' ? 'Voucher Detail' : 'Add Voucher'"
                ...
                <div class="grid grid-cols-2 gap-6 mt-4">
                    ...
                    <div>
                        <InputForm id="discount_amount" v-model="form.discount_amount" label="Discount Amount (%)"
                            ...
                            :error="formError.discount_amount" :readonly="mode === 'view'" />
                    ...
                    <!-- Start Date -->
                    <InputForm id="start_date" v-model="form.start_date" label="Start Date" type="date" required
                        :error="formError.start_date" :readonly="mode === 'view'" />

                    <!-- End Date -->
                    <InputForm id="end_date" v-model="form.end_date" label="End Date" type="date" required
                        :error="formError.end_date" :readonly="mode === 'view'" />

                    <!-- Active Status -->
                    <ToggleForm v-model="form.is_active" label="Active Status" :disabled="mode === 'view'" />
                ...
                <div v-if="mode === 'view'" class="mt-6 p-4 bg-gray-100 rounded-lg shadow-sm">
                    <h3 class="text-lg font-semibold text-gray-700">Total Sold</h3>
                    <p class="text-2xl font-bold text-pinkDark">{{ form.totalSold }} vouchers sold</p>
                </div>
...
const form = ref({
    ...
    store_id: JSON.parse(Cookies.get('userdata')).store_id,
...
onMounted(async () => {
    if (mode.value !== 'add' && id.value) {
        try {
            const response = await axiosInstance.get(`http://127.0.0.1:3000/transaction/voucher/${id.value}/id`, {
