...
const columns = [
  ...
  { data: 'trans_start_date', title: 'Start Date', render: (data) => new Date(data).toLocaleDateString() },
  { data: 'trans_last_date', title: 'Last Date', render: (data) => data != null ? new Date(data).toLocaleDateString() : '-' },
...
<template>
  <div class="content" :class="{ 'full-width': smallMenu }">
    <PageTitle
      :title="'List of Recurring Income/Expense'"
    />
    <TableData 
      :columns="columns"
      ...
      :deletePath="'/finance/recurring'"
	    :infoPath="'/finance/recurring/view'"
    />
  </div>
</template>