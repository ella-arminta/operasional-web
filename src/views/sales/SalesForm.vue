...
watch(
	...
	(newValue, oldValue) => {
		...
		newValue.forEach((item) => {
			const itemTotal = parseFloat(item.price) * parseFloat(item.quantity) + parseFloat(item.adjustment_price)
...
// 		form.value.tax_price = total * parseFloat(tax.value / 100)
// 		if (form.value.payment_link) {
// 			// For marketplace transactions, don't calculate total_price
// 			console.log('Marketplace transaction - using API total_price')
// 		} else {
// 			form.value.total_price = total + parseFloat(form.value.tax_price)
// 		}
