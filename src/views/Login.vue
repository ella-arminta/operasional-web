<template>
	<div class="app flex justify-center items-center min-h-screen bg-pinkGray">
		<div class="login-container w-full max-w-md bg-white p-8 rounded-xl shadow-lg transform transition-all duration-500 ease-in-out"
			:class="{ 'scale-in': showForm }">
			<!-- Logo dengan animasi -->
			<div class="flex justify-center mb-8">
				<img src="/logo.png" alt="Logamas Logo"
					class="w-auto h-36 object-contain transition-all duration-500 hover:scale-105"
					@mouseover="logoHover = true" @mouseleave="logoHover = false" :class="{ 'pulse': logoHover }" />
			</div>

			<!-- Alert notifikasi -->
			<div v-if="alert.show" class="mb-6 p-4 rounded-lg border-l-4 transition-all duration-300 ease-in-out"
				:class="alertStyles">
			...
			<form @submit.prevent="submit" class="space-y-5">
				...
				<div class="form-group relative">
					<label for="email" class="block text-sm font-medium text-gray-700 mb-1 transition-all duration-300"
						:class="{ 'text-pinkDark': focusedField === 'email' }">Email</label>
					<div class="relative">
						<input v-model="data.email" id="email" type="email" placeholder="Enter your email"
							...
							:class="[
								focusedField === 'email' ? 'border-pinkDark ring-2 ring-pink-100 bg-white' : 'border-gray-200'
							]" @focus="focusField('email')" @blur="blurField" />
						<span class="absolute right-3 top-3 text-gray-400">
							<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
								:class="{ 'text-pinkDark': focusedField === 'email' }" fill="none" viewBox="0 0 24 24"
								stroke="currentColor">
				...
				<div class="form-group relative">
					<label for="password"
						...
						:class="{ 'text-pinkDark': focusedField === 'password' }">Password</label>
					<div class="relative">
						<input v-model="data.password" id="password" :type="showPassword ? 'text' : 'password'"
							placeholder="Enter your password"
							class="block w-full px-4 py-3 bg-gray-50 border rounded-lg focus:outline-none transition-all duration-300 ease-in-out"
							:class="[
								focusedField === 'password' ? 'border-pinkDark ring-2 ring-pink-100 bg-white' : 'border-gray-200'
							]" @focus="focusField('password')" @blur="blurField" />
						<button type="button"
				...
				<div class="pt-2">
					<button type="submit" :class="[
						...
						isLoading ? 'bg-pinkDarker cursor-not-allowed' : 'bg-pinkDark hover:bg-pinkDarker hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 focus:outline-none focus:ring-2 focus:ring-pinkOrange'
					]" :disabled="isLoading">
						<span v-if="!isLoading" class="text-white relative z-10">Sign In</span>
						...
						<span class="ripple absolute bg-white/20 rounded-full transform scale-0 transition-transform"
							:style="rippleStyle" ref="ripple"></span>
					</button>
				</div>
			</form>
		</div>
				...
				if (response.data) {
					...
					if (
						...
						if (response.data.is_owner) {
							...
							setTimeout(() => router.push('/onboarding/employee'), 1500)
							return
						}
					} else {
