    ...
    <div class="content min-h-screen" :class="{ 'full-width': smallMenu }">
        ...
        <div class="w-full bg-white h-auto rounded-lg shadow-sm py-3 px-4">
            <form @submit.prevent="submit">
                ...
                <FormSectionHeader title="Customer Information" icon="user" />

                <div class="grid grid-cols-1 gap-6 mt-4">
                    <!-- Name -->
                    <InputForm id="name" v-model="form.name" label="Name" placeholder="Enter name" required
                        :error="formError.name" />

                    <!-- Email -->
                    <InputForm id="email" v-model="form.email" label="Email" type="email" placeholder="Enter email"
                        required :error="formError.email" />

                    <!-- Phone -->
                    <InputForm id="phone" v-model="form.phone" label="Phone" type="tel" placeholder="Enter phone number"
                        required :error="formError.phone" />
                </div>
                ...
                <p class="mt-2 text-sm text-gray-500">
                    * Default password for new customers is <strong>"password"</strong>.
                </p>
            </form>
        </div>
    </div>
...
const submit = async () => {
    ...
    try {
        const response = await axiosInstance.post("http://127.0.0.1:3001/api/user/add-by-admin", form.value);
